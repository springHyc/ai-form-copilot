...
- **Mock / 填充（RangePicker + showTime）**：new-market 榕树运营管理系统「首页弹窗-有效时间」`<RangePicker showTime={{ format: 'HH:mm:ss' }} format='YYYY-MM-DD HH:mm:ss' />`（`add-info.tsx:360-362`）此前只填上年月日、时分秒全丢，页面仍提示「请选择有效时间」。根因：`fillDateRange` 直写路径把 value 截成 `YYYY-MM-DD`，下拉路径点完日期也没走时间列 + OK。现改为：Mock `randomDateRange` 直接生成 `YYYY-MM-DD HH:mm:ss,YYYY-MM-DD HH:mm:ss`（分钟仅 00/30，时段 10–17）；`fillDateRange` 先打开面板逐日点选，出现 `.ant-picker-time-panel` / `.ant-picker-ok` 即为 `showTime`，追加 `tryPickTimeInDropdown` + `confirmPickerIfNeeded`（开始/结束各一次）；直写兜底按 placeholder（`时分秒 / HH:mm / 时间`）判定是否保留 `HH:mm:ss`，普通日期 RangePicker 自动回退为 `YYYY-MM-DD` 避免把冗余时间丢给 rc-picker 触发 parse 失败；AI 规则 6 同步补充 daterange 的带时分秒格式。
