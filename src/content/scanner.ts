...
function detectFieldType(container: HTMLElement): { type: FieldType; element: HTMLElement } | null {
...
function extractLabel(container: HTMLElement): string {
  ...
  const labelEl = container.querySelector(':scope > .ant-row > .ant-form-item-label label')
    ?? container.querySelector(':scope > .ant-form-item-label label')
    ?? container.querySelector('.ant-form-item-label label');
  if (labelEl) {
    return (labelEl.textContent?.trim() ?? '').replace(/^\*\s*/, '').replace(/[：:]$/, '');
  }

  // 兜底：直接查找直属 label 标签
  const fallbackLabel = container.querySelector(':scope > .ant-row label')
    ?? container.querySelector('label');
  if (fallbackLabel) {
    return (fallbackLabel.textContent?.trim() ?? '').replace(/^\*\s*/, '').replace(/[：:]$/, '');
  }
...
/** 提取字段约束信息（InputNumber 取 .ant-input-number-input 上的 min/max/step） */
function extractConstraints(element: HTMLElement, fieldType: FieldType): FormFieldInfo['constraints'] | undefined {
...
export function scanFormFields(): FormFieldInfo[] {
  ...
  console.log(`[AI Form Copilot] 找到 ${formItems.length} 个顶层 .ant-form-item 容器`);
